...
/* delete 语句序列化 */
bool ripple_fftrail_txndelete_serial(void* data, void* state)
{
    /*
     * Record格式为:
     *  GroupToken
...
ripple_fftrail_txndelete_serial_retry:
    do
    {
        /* 将表写入到 trail 文件中 */
    ...
    if(RIPPLE_FFSMGR_STATUS_SHIFTFILE == ffstate->status)
    {
        ...
        ffstate->status = RIPPLE_FFSMGR_STATUS_USED;
        goto ripple_fftrail_txndelete_serial_retry;
    }
    ...
    txndata->header.subtype = RIPPLE_FF_DATA_TYPE_DML_DELETE;
...
/* delete信息反序列化 */
bool ripple_fftrail_txndelete_deserial(void** data, void* state)
{
    bool    found = false;
    ...
    *data = txndata;
    ...
    tbcolbase.m_dmltype = XK_PG_PARSER_TRANSLOG_DMLTYPE_DELETE;
