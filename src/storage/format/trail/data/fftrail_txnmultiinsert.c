...
/* multiinsert语句序列化 */
bool ripple_fftrail_txnmultiinsert_serial(void* data, void* state)
{
    int hdrlen = 0;
    uint32 dbmdno = 0;
    uint32 tbmdno = 0;
...
ripple_fftrail_txnmultiinsert_serial_retry:
    do
    {
        /* 将表写入到 trail 文件中 */
    ...
    if(RIPPLE_FFSMGR_STATUS_SHIFTFILE == ffstate->status)
    {
        ...
        ffstate->status = RIPPLE_FFSMGR_STATUS_USED;
        goto ripple_fftrail_txnmultiinsert_serial_retry;
    }
    ...
    txndata->header.subtype = RIPPLE_FF_DATA_TYPE_DML_MULTIINSERT;
...
/*multiinsert信息反序列化 */
bool ripple_fftrail_txnmultiinsert_deserial(void** data, void* state)
{
    bool    found = false;
