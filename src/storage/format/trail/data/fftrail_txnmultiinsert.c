...
/* Serialize multiinsert statement */
...
fftrail_txnmultiinsert_serial_retry:
    do
    {
        ...
        fftrail_tbmetadata_serial(false,
                                  ...
                                  colnvalues->m_relid, txndata->header.transid, &dbmdno, &tbmdno,
                                  state);
    ...
    /* Get values column info based on m_rowCnt */
    for (i = 0; i < colnvalues->m_rowCnt; i++)
    {
        for (j = 0; j < colnvalues->m_valueCnt; j++)
        {
            ...
            /* colid(2) */
            fftrail_data_data2buffer(&txndata->header, ffstate, &fbuffer,
            ...
            /* (length 4) */
            fftrail_data_data2buffer(&txndata->header, ffstate, &fbuffer, FTRAIL_TOKENDATATYPE_INT,
                                     4, (uint8*)&col.m_valueLen);

            if (0 != col.m_valueLen)
            {
                /* data* */
                fftrail_data_data2buffer(&txndata->header, ffstate, &fbuffer,
                                         FTRAIL_TOKENDATATYPE_STR, col.m_valueLen,
                                         (uint8*)col.m_value);
            }
        }
    }
...
bool fftrail_txnmultiinsert_deserial(void** data, void* state)
{
    ...
    *data = txndata;
     ...
     *  2. Data assembly
     */
    /* Get each row info based on txndata->header.reccount */
    for (i = 0; i < colnvalues->m_rowCnt; i++)
    {
        /* Allocate space for each row column info */
        ...
        /* Get multiinsert column info */
        while (0 < totallen)
        {
            /* Get colid */
            if (false == fftrail_data_buffer2data(&txndata->header, ffstate, &recoffset,
                                                  &dataoffset, FTRAIL_TOKENDATATYPE_SMALLINT, 2,
                                                  (uint8*)&colid))
            {
                return false;
            }
