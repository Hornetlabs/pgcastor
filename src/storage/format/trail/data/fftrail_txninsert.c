...
fftrail_txninsert_serial_retry:
    do
    {
        /* Write table to trail file */
        fftrail_tbmetadata_serial(false, rstmt->database, colvalues->m_relid,
                                  txndata->header.transid, &dbmdno, &tbmdno, state);
    ...
    if (FFSMGR_STATUS_SHIFTFILE == ffstate->status)
    {
        ...
        ffstate->status = FFSMGR_STATUS_USED;
        goto fftrail_txninsert_serial_retry;
    }
...
bool fftrail_txninsert_deserial(void** data, void* state)
{
    ...
    *data = txndata;
    ...
    tbcolbase.m_dmltype = PG_PARSER_TRANSLOG_DMLTYPE_INSERT;
    ...
    colvalues->m_relfilenode = 0;
    colvalues->m_old_values = NULL;
    rstmt->len = txndata->header.totallength;
    rstmt->len -= (colvalues->m_valueCnt * 8);
