...
ripple_fftrail_txninsert_serial_retry:
    do
    {
        /* 将表写入到 trail 文件中 */
    ...
    if(RIPPLE_FFSMGR_STATUS_SHIFTFILE == ffstate->status)
    {
        ...
        ffstate->status = RIPPLE_FFSMGR_STATUS_USED;
        goto ripple_fftrail_txninsert_serial_retry;
    }
    ...
    txndata->header.subtype = RIPPLE_FF_DATA_TYPE_DML_INSERT;
...
bool ripple_fftrail_txninsert_deserial(void** data, void* state)
{
    ...
    *data = txndata;
    ...
    tbcolbase.m_dmltype = XK_PG_PARSER_TRANSLOG_DMLTYPE_INSERT;
