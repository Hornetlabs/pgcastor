...
bool fftrail_txndelete_serial(void* data, void* state)
{
    /*
     * Record format:
     *  GroupToken
...
fftrail_txndelete_serial_retry:
    do
    {
    ...
    if (FFSMGR_STATUS_SHIFTFILE == ffstate->status)
    {
        ...
        ffstate->status = FFSMGR_STATUS_USED;
        goto fftrail_txndelete_serial_retry;
    }
    ...
    txndata->header.subtype = FF_DATA_TYPE_DML_DELETE;
...
bool fftrail_txndelete_deserial(void** data, void* state)
{
    ...
    *data = txndata;
    ...
    tbcolbase.m_dmltype = PG_PARSER_TRANSLOG_DMLTYPE_DELETE;
