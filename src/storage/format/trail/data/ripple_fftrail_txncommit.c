 ...
 * 
 * metadata数据不序列化，当最后一条为metadata加条commit强制结束
 */
bool ripple_fftrail_txncommit_serial(void* data, void* state)
{
    ...
    /* 计算长度 */
    txndata->header.totallength = 8;           /* 语句的长度             */

    /* 设置record头信息 */
    txndata->header.reccount = 1;
    txndata->header.reclength = 0;
    txndata->header.subtype = RIPPLE_FF_DATA_TYPE_TXNCOMMIT;
...
bool ripple_fftrail_txncommit_deserial(void** data, void* state)
{
    ...
    *data = txndata;
    ...
    /* 申请空间 */
    commit = (ripple_commit_stmt*)rmalloc0(sizeof(ripple_commit_stmt));
    if(NULL == commit)
    {
        elog(RLOG_ERROR, "out of memory, %s", strerror(errno));
    }
    rmemset0(commit, 0, '\0', sizeof(ripple_commit_stmt));
    rstmt->type = RIPPLE_TXNSTMT_TYPE_COMMIT;
