...
ripple_task_refresh_p2csharding *ripple_refresh_p2csharding_init(void)
{
    ripple_task_refresh_p2csharding *p2csharding = NULL;
    p2csharding = rmalloc0(sizeof(ripple_task_refresh_p2csharding));
...
/* 创建 p2c identity 数据包 */
static bool ripple_refresh_p2csharding_p2cidentity(ripple_netclient* netclient)
{
    uint32 namelen = 0;
    ...
    if (NULL == netpacket)
    {
        elog(RLOG_WARNING, "init refresh identity packet error");
        goto ripple_refresh_p2csharding_p2cidentity_error;
    }
    ...
    {
        elog(RLOG_WARNING, "init refresh identity data error");
        goto ripple_refresh_p2csharding_p2cidentity_error;
    }
    ...
    RIPPLE_CONCAT(put, 8bit)(&wuptr, RIPPLE_NETIDENTITY_TYPE_PUMPREFRESHARDING);
    ...
    {
        elog(RLOG_WARNING, "refresh add identity packet error");
        goto ripple_refresh_p2csharding_p2cidentity_error;
    }
    return true;

ripple_refresh_p2csharding_p2cidentity_error:
    ripple_netpacket_destroy(netpacket);
    return false;
}

/* 创建 beginrefresh 数据包挂载到 wpackets 上 */
static void ripple_refresh_p2csharding_wpacketsadd_begin(ripple_netclient* netclient, char* schema, char* table, uint32 shards, uint32 shardnum)
{
    uint16 tblen = 0;
    uint16 schlen = 0;
    uint32 wmsglen = 0;
    ...
    wmsglen = (schlen + tblen + RIPPLE_NETMSG_TYPE_P2C_BEGINREFRESH_SIZE);
    ...
    /* 发送请求，获取标识 */
    wuptr = netpacket->data;
    RIPPLE_CONCAT(put, 32bit)(&wuptr, RIPPLE_NETMSG_TYPE_P2C_BEGINREFRESH);
    ...
    elog(RLOG_INFO, "pump begin refresh");
}

/* 创建 发送数据包挂载到 wpackets 上 */
static void ripple_refresh_p2csharding_wpacketsadd_data(ripple_netclient* netclient, uint64 offset, char* read_buffer, size_t read_size)
{
    uint32 wmsglen = 0;
    ...
    wmsglen = RIPPLE_NETMSG_TYPE_P2C_REFRESHDATA_FIXSIZE + read_size;
    ...
    /* 发送请求，获取标识 */
    wuptr = netpacket->data;
    RIPPLE_CONCAT(put, 32bit)(&wuptr, RIPPLE_NETMSG_TYPE_P2C_REFRESHDATA);
    RIPPLE_CONCAT(put, 32bit)(&wuptr, wmsglen);
    ...
    rmemset0(read_buffer, 0, '\0', RIPPLE_FILE_BUFFER_SIZE + RIPPLE_NETMSG_TYPE_P2C_REFRESHDATA_FIXSIZE);

    ripple_netclient_addwpacket(netclient, (void*)netpacket);
}

/* 创建 endrefresh 数据包挂载到 wpackets 上 */
static void ripple_refresh_p2csharding_wpacketsadd_end(ripple_netclient* netclient)
{
    uint8* wuptr = NULL;
    ripple_netpacket* netpacket = NULL;
    netpacket = ripple_netpacket_init();
    netpacket->max = RIPPLE_MAXALIGN(RIPPLE_NETMSG_TYPE_P2C_ENDREFRESH_SIZE);
    netpacket->offset = RIPPLE_NETMSG_TYPE_HDR_SIZE;
    netpacket->used = RIPPLE_NETMSG_TYPE_P2C_ENDREFRESH_SIZE;
    netpacket->data = ripple_netpacket_data_init(netpacket->max);

    /* 发送请求，获取标识 */
    wuptr = netpacket->data;
    RIPPLE_CONCAT(put, 32bit)(&wuptr, RIPPLE_NETMSG_TYPE_P2C_ENDREFRESH);
    RIPPLE_CONCAT(put, 32bit)(&wuptr, RIPPLE_NETMSG_TYPE_P2C_ENDREFRESH_SIZE);

    ripple_netclient_addwpacket(netclient, (void*)netpacket);
}

/* 重置待发送文件信息 */
static bool ripple_refresh_p2csharding_resetfileinfo(ripple_refresh_pumpshardingnetstate* pnet,
                                                    ripple_refresh_table_sharding* tblsharding)
{
    elog(RLOG_DEBUG, "refresh worker: %s.%s %4d %4d",
                    tblsharding->schema,
                    tblsharding->table,
...
void* ripple_refresh_p2csharding_main(void* args)
{
    ...
    ripple_task_refresh_p2csharding *shard_slot         = NULL;
    ...
    shard_slot = (ripple_task_refresh_p2csharding *)thrnode->data;

    /* 查看状态 */
    if(RIPPLE_THRNODE_STAT_STARTING != thrnode->stat)
    {
        elog(RLOG_WARNING, "pump refresh job stat exception, expected state is RIPPLE_THRNODE_STAT_STARTING");
    ...
    if(NULL == clientstate)
    {
        elog(RLOG_WARNING, "pump refresh job int net error");
        thrnode->stat = RIPPLE_THRNODE_STAT_ABORT;
        ripple_pthread_exit(NULL);
    }
    ...
    /* refresh 目录 */
    snprintf(clientstate->refeshpath, RIPPLE_MAXPATH, "%s", guc_getConfigOption(RIPPLE_CFG_KEY_TRAIL_DIR));

    /* 申请空间，用于读数据 */
    read_buffer = rmalloc0(RIPPLE_FILE_BUFFER_SIZE + RIPPLE_NETMSG_TYPE_P2C_REFRESHDATA_FIXSIZE);
    if(NULL == read_buffer)
    {
    ...
    while(RIPPLE_REFRESH_PUMPSHARDINGNETSTATE_STATE_NOP == clientstate->state)
    {
        /* 退出 */
        ...
        /* 添加身份验证 */
        if (false == ripple_refresh_p2csharding_p2cidentity(netclient))
        {
            elog(RLOG_INFO, "add refresh identity packet error");
            sleep(1);
            continue;
        }
        ...
        while (1)
        {
            ...
            if (RIPPLE_REFRESH_PUMPSHARDINGNETSTATE_STATE_IDLE == clientstate->state)
            {
                ...
                if(NULL == qitem_data)
                {
                    ...
                    /* 出错了 */
                    elog(RLOG_WARNING, "pump refresh job get file from queue error");
                    thrnode->stat = RIPPLE_THRNODE_STAT_ABORT;
                    goto ripple_refresh_p2csharding_main_done;
                }

                /* 获取到了数据 */
                thrnode->stat = RIPPLE_THRNODE_STAT_WORK;
                shard = (ripple_refresh_table_sharding *)qitem_data;
                if(false == ripple_refresh_p2csharding_resetfileinfo(clientstate, shard))
                {
                    /* 睡眠一秒, 防止出现打印大量日志 */
                    sleep(1);
                    if (NULL != qitem_data)
            ...
            {
                ripple_refresh_p2csharding_wpacketsadd_begin(netclient,
                                                            shard->schema,
                                                            shard->table,
                                                            shard->shardings,
                                                            shard->sharding_no);
            ...
            if (RIPPLE_REFRESH_PUMPSHARDINGNETSTATE_STATE_READFILE == clientstate->state)
            {
                ...
                if (0 == clientstate->filesize)
                {
                    ...
                    clientstate->fd = -1;
                    ripple_refresh_p2csharding_wpacketsadd_end(netclient);
                    clientstate->base.hbtimeout = 0;
                    clientstate->state = RIPPLE_REFRESH_PUMPSHARDINGNETSTATE_STATE_END;
                }
                else
                {
                    elog(RLOG_DEBUG, "pump refresh worker, queue: %s.%s %4d %4d, file size: %u, read/send size: %u",
                                                            shard->schema,
                                                            shard->table,
                                                            shard->shardings,
                                                            shard->sharding_no,
            ...
            {
                ripple_refresh_p2csharding_wpacketsadd_data(netclient,
...
ripple_refresh_p2csharding_main_done:
...
void ripple_refresh_p2csharding_free(void* args)
{
    ...
    shard_slot = (ripple_task_refresh_p2csharding *)args;
