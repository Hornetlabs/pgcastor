...
static void ripple_netmsg_p2cendrefresh_filetransfer_add(ripple_increment_collectornetclient_state* nodesvrstate)
{
    ...
    ripple_filetransfer_upload_refreshpath_set(filetransfer_refresh, nodesvrstate->clientname);
...
static bool ripple_netmsg_p2cendrefresh_mvfile(ripple_increment_collectornetclient_state* nodesvrstate);

bool ripple_netmsg_p2cendrefresh(void* privdata, uint8* msg)
{
    /*
     * 向 pump 发送结束信息
     */
    ...
    int event = 0;
    uint8* uptr = NULL;
    uint8   c2pendrefresh[RIPPLE_NETMSG_TYPE_C2P_ENDREFRESH_SIZE] = { 0 };
    ...
    if (!ripple_netmsg_p2cendrefresh_mvfile(nodesvrstate))
    {
        return false;
    }

    uptr = c2pendrefresh;
    RIPPLE_CONCAT(put, 32bit)(&uptr, RIPPLE_NETMSG_TYPE_C2P_ENDREFRESH);
    RIPPLE_CONCAT(put, 32bit)(&uptr, RIPPLE_NETMSG_TYPE_C2P_ENDREFRESH_SIZE);
    ...
    while(1)
    {
        ...
        {
            /* 写数据 */
            if(false == ripple_net_write(nodesvrstate->fd, c2pendrefresh, RIPPLE_NETMSG_TYPE_C2P_ENDREFRESH_SIZE))
            {
                /* 发送数据失败，关闭连接 */
                elog(RLOG_WARNING, "write RIPPLE_NETMSG_TYPE_C2P_ENDREFRESH_SIZE error, %s", strerror(errno));
                return true;
            }

            nodesvrstate->hbtimeout = 0;
            elog(RLOG_INFO, "collector 2 pump endrefresh");
            break;
        }
    }
...
static bool ripple_netmsg_p2cendrefresh_mvfile(ripple_increment_collectornetclient_state* nodesvrstate)
{
    ...
    appendStringInfo(completepath, "%s/%s_%s/%s",
    ...
    /* 生成filetransfer节点，并添加到任务中 */
    ripple_netmsg_p2cendrefresh_filetransfer_add(nodesvrstate);
