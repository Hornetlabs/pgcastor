...
/* 
 * 接收来自collector的endrefres请求
 *  pump 处理
 */
bool ripple_netmsg_c2pendrefresh(void* privdata, uint8* msg)
{
    /*
     * 更新地址信息
     */
    ...
    if (RIPPLE_NETMSG_TYPE_C2P_ENDREFRESH == msgtype)
    {
        /* 接收到endrefresh设置状态为IDLE读取新的数据 */
        clientstate->state = RIPPLE_REFRESH_PUMPSHARDINGNETSTATE_STATE_IDLE;
    }
    else
    {
    ...
    elog(RLOG_INFO, "pump end refresh");
    return true;
}
