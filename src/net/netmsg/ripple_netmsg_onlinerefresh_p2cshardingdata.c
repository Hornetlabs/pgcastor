...
bool ripple_netmsg_onlinerefresh_p2cshardingdata(void* privdata, uint8* msg)
{
    uint32 msglen = 0;
    uint64 offset = 0;
    ...
    ripple_collectornetclient_onlinerefreshsharding* refreshshardingstate = NULL;

    uptr = msg;
    nodesvrstate = (ripple_increment_collectornetclient_state*)privdata;
    refreshshardingstate = (ripple_collectornetclient_onlinerefreshsharding*)nodesvrstate->data;
    ...
    /* 获取固定的附加信息 */
    msglen -= RIPPLE_NETMSG_TYPE_P2C_ONLINEREFRESH_SHARDING_DATA_FIXSIZE;
    if (0 == msglen)
    {
        return true;
    }
