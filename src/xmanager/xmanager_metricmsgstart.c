 ...
 */
bool xmanager_metricmsg_parsestart(xmanager_metric* xmetric, netpoolentry* npoolentry, netpacket* npacket)
{
    int                        len = 0;
    int                        jobtype = 0;
    int                        errcode = 0;
    ...
    {
        errcode = ERROR_OOM;
        snprintf(errormsg, 2048, "ERROR: xmanager parse start command, out of memory.");
        goto xmanager_metricmsg_parsestart_error;
    }
    ...
    {
        errcode = ERROR_OOM;
        snprintf(errormsg, 2048, "ERROR: %s", "xmanager recv start command, out of memory");
        goto xmanager_metricmsg_parsestart_error;
    }
    ...
    {
        snprintf(execcmd, 1024, "%s/bin/pgreceivelog/receivelog -f %s start", xmetric->pgcastorpath, pxmetricnode->conf);
    }
    else
    {
        snprintf(execcmd,
    ...
    {
        errcode = ERROR_MSGCOMMAND;
        snprintf(errormsg, 2048, "ERROR: can not start %s", xmanager_metricnode_getname(jobtype));
...
xmanager_metricmsg_parsestart_error:
    ...
    return xmanager_metricmsg_assembleerrormsg(xmetric, npoolentry->wpackets, XMANAGER_MSG_STARTCMD, errcode, errormsg);
}

/*
 * Assemble start response message
 */
bool xmanager_metricmsg_assemblestart(xmanager_metric* xmetric, netpoolentry* npoolentry, dlist* dlmsgs)
{
    uint8                    u8value = 0;
    uint16                   u16value = 0;
    int                      rowlen = 0;
    ...
    if (NULL == npacket)
    {
        elog(RLOG_WARNING, "xmanager metric assemble start msg out of memory");
        return false;
    }
    msglen += 1;
    ...
    {
        elog(RLOG_WARNING, "xmanager metric assemble start msg data, out of memory");
