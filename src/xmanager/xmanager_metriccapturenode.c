 ...
 * namelen 4 + parsetimestamp
 */
#define REFRESH_METRICCAPTURE_KEY_LEN ( 4 + 4 + strlen("redolsn") + 4 + strlen("restartlsn") + 4 + strlen("confirmlsn") + 4 + strlen("loadlsn") + 4 + strlen("parselsn") + 4 + strlen("flushlsn") + 4 + strlen("trailno") + 4 + strlen("trailstart") + 4 + strlen("parsetimestamp") + 4 + strlen("parsetimestamp"))
...
void* xmanager_metricmsg_assemblecapture(xmanager_metricnode* pxmetricnode)
{
    ...
    /* redolsn + len */
    msglen += 4;
    valuelen[0] = snprintf(values[0], 32, "%X/%X", (uint32)(xmetriccapturenode->redolsn >> 32), (uint32)(xmetriccapturenode->redolsn));
    msglen += valuelen[0];

    /* restartlsn + len */
    msglen += 4;
    valuelen[1] = snprintf(values[1], 32, "%X/%X", (uint32)(xmetriccapturenode->restartlsn >> 32), (uint32)(xmetriccapturenode->restartlsn));
    msglen += valuelen[1];

    /* confirmlsn + len */
    msglen += 4;
    valuelen[2] = snprintf(values[2], 32, "%X/%X", (uint32)(xmetriccapturenode->confirmlsn >> 32), (uint32)(xmetriccapturenode->confirmlsn));
    msglen += valuelen[2];

    /* loadlsn + len */
    msglen += 4;
    valuelen[3] = snprintf(values[3], 32, "%X/%X", (uint32)(xmetriccapturenode->loadlsn >> 32), (uint32)(xmetriccapturenode->loadlsn));
    msglen += valuelen[3];

    /* parselsn + len */
    msglen += 4;
    valuelen[4] = snprintf(values[4], 32, "%X/%X", (uint32)(xmetriccapturenode->parselsn >> 32), (uint32)(xmetriccapturenode->parselsn));
    msglen += valuelen[4];

    /* flushlsn + len */
    msglen += 4;
    valuelen[5] = snprintf(values[5], 32, "%X/%X", (uint32)(xmetriccapturenode->flushlsn >> 32), (uint32)(xmetriccapturenode->flushlsn));
    msglen += valuelen[5];
