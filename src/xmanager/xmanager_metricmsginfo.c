                                         ...
                                         netpacket* npacket)
{
    ...
    if (XMANAGER_METRICNODETYPE_PROCESS < jobtype)
    {
        ...
        snprintf(errormsg, 512, "ERROR: xmanager info command, temporarily unsupport %s", xmanager_metricnode_getname(jobtype));
    ...
    /* 组装返回信息 */
    if(false == xmanager_metricmsg_assembleinforesult(xmetric,
                                                             npoolentry,
                                                             pxmetricnode))
    {
        errcode = ERROR_OOM;
