...
/*  info 结果 组装 */
static bool ripple_xmanager_metricmsg_assembleinforesult(ripple_xmanager_metric* xmetric,
                                                         ...
                                                         ripple_xmanager_metricnode* pxmetricnode)
{
    ...
    case RIPPLE_XMANAGER_METRICNODETYPE_PROCESS:
        ...
        break;
    default:
        elog(RLOG_WARNING, "xmanager metric assemble info  msg data, unsupported metricnode type :%d", pxmetricnode->type);
        return false;
        break;
    }
 ...
 *  3、获取信息并返回
*/
bool ripple_xmanager_metricmsg_parseinfo(ripple_xmanager_metric* xmetric,
                                         ripple_netpoolentry* npoolentry,
                                         ripple_netpacket* npacket)
{
    ...
    if (RIPPLE_XMANAGER_METRICNODETYPE_PROCESS < jobtype)
    {
        ...
        snprintf(errormsg, 512, "ERROR: xmanager info command, temporarily unsupport %s", ripple_xmanager_metricnode_getname(jobtype));
        goto ripple_xmanager_metricmsg_parseinfo_error;
    }
    ...
    /* 组装返回信息 */
    if(false == ripple_xmanager_metricmsg_assembleinforesult(xmetric,
                                                             npoolentry,
                                                             pxmetricnode))
    {
        errcode = RIPPLE_ERROR_OOM;
...
ripple_xmanager_metricmsg_parseinfo_error:
