...
/*------------------pump parse begin----------------------------*/
static bool ripple_xmanager_metricmsg_parsepumpincmsg(ripple_xmanager_metric* xmetric,
                                                      ripple_netpoolentry* npoolentry,
                                                      ripple_netpacket* npacket)
{
    int len                                                 = 0;
    uint8* uptr                                             = NULL;
    ripple_xmanager_metricpumpnode* pxmetricpumpnode  = NULL;
    ripple_xmanager_metricpumpnode xmetricpumpnode     = { {0} };
    ...
    /* jobname */
    xmetricpumpnode.base.name = (char*)uptr;
    uptr += len;

    /* 继续获取 redolsn */
    rmemcpy1(&xmetricpumpnode.loadlsn, 0, uptr, 8);
    uptr[0] = '\0';
    uptr += 8;

    /* 根据名称获取 metricnode */
    xmetricpumpnode.base.type = RIPPLE_XMANAGER_METRICNODETYPE_PUMP;
    pxmetricpumpnode = dlist_get(xmetric->metricnodes, &xmetricpumpnode, ripple_xmanager_metricnode_cmp);
    if (NULL == pxmetricpumpnode)
    {
        elog(RLOG_WARNING, "xmanager metric pump msg can not get pump by name:%s", xmetricpumpnode.base.name);
        return false;
    }

    /* 设置 redolsn */
    pxmetricpumpnode->loadlsn = xmetricpumpnode.loadlsn;
    pxmetricpumpnode->loadlsn = r_ntoh64(pxmetricpumpnode->loadlsn);

    /* 设置 sendlsn */
    rmemcpy1(&pxmetricpumpnode->sendlsn, 0, uptr, 8);
    pxmetricpumpnode->sendlsn = r_ntoh64(pxmetricpumpnode->sendlsn);
    uptr += 8;

    /* 获取 loadtrailno */
    rmemcpy1(&pxmetricpumpnode->loadtrailno, 0, uptr, 8);
    pxmetricpumpnode->loadtrailno = r_ntoh64(pxmetricpumpnode->loadtrailno);
    uptr += 8;

    /* 获取 loadtrailstart */
    rmemcpy1(&pxmetricpumpnode->loadtrailstart, 0, uptr, 8);
    pxmetricpumpnode->loadtrailstart = r_ntoh64(pxmetricpumpnode->loadtrailstart);
    uptr += 8;

    /* 获取 sendtrailno */
    rmemcpy1(&pxmetricpumpnode->sendtrailno, 0, uptr, 8);
    pxmetricpumpnode->sendtrailno = r_ntoh64(pxmetricpumpnode->sendtrailno);
    uptr += 8;

    /* 获取 sendtrailstart */
    rmemcpy1(&pxmetricpumpnode->sendtrailstart, 0, uptr, 8);
    pxmetricpumpnode->sendtrailstart = r_ntoh64(pxmetricpumpnode->sendtrailstart);
    uptr += 8;

    /* 获取 loadtimestamp */
    rmemcpy1(&pxmetricpumpnode->loadtimestamp, 0, uptr, 8);
    pxmetricpumpnode->loadtimestamp = r_ntoh64(pxmetricpumpnode->loadtimestamp);
    uptr += 8;

    /* 获取 sendtimestamp */
    rmemcpy1(&pxmetricpumpnode->sendtimestamp, 0, uptr, 8);
    pxmetricpumpnode->sendtimestamp = r_ntoh64(pxmetricpumpnode->sendtimestamp);
    uptr += 8;
...
/*------------------collector parse begin----------------------------*/
static bool ripple_xmanager_metricmsg_parsecollectorincmsg(ripple_xmanager_metric* xmetric,
                                                           ripple_netpoolentry* npoolentry,
                                                           ripple_netpacket* npacket)
{
    int len                                                     = 0;
    ...
    int idx_pump                                                = 0;
    uint8* uptr                                                 = NULL;
    ripple_xmanager_metriccollectornode* pxmetriccollectornode  = NULL;
    ripple_xmanager_metriccollectorinfo* xmetriccollectorinfo   = NULL;
    char pumpmane[128]                                          = { 0 };
    ripple_xmanager_metriccollectornode xmetriccollectornode    = { {0} };
    ...
    rmemcpy1(&pumpcnt, 0, uptr, 4);
    pumpcnt = r_ntoh32(pumpcnt);
    uptr += 4;
    ...
    /* jobname */
    xmetriccollectornode.base.name = (char*)uptr;
    uptr += len;

    /* 根据名称获取 metricnode */
    xmetriccollectornode.base.type = RIPPLE_XMANAGER_METRICNODETYPE_COLLECTOR;
    pxmetriccollectornode = dlist_get(xmetric->metricnodes, &xmetriccollectornode, ripple_xmanager_metricnode_cmp);
    if (NULL == pxmetriccollectornode)
    {
        elog(RLOG_WARNING, "xmanager metric collector msg can not get collector by name:%s", xmetriccollectornode.base.name);
        return false;
    }
    ...
    {
        /* 获取 jobnamelen */
        rmemset1(pumpmane, 0, '\0', 128);
        rmemcpy1(&len, 0, uptr, 4);
        len = r_ntoh32(len);
        uptr += 4;

        /* jobname */
        rmemcpy1(pumpmane, 0, uptr, len);
        uptr += len;

        /* 检测pumpname在 collectorinfo 中是否存在 */
        xmetriccollectorinfo = dlist_isexist(pxmetriccollectornode->collectorinfo, pumpmane, ripple_xmanager_metriccollectorinfo_cmp);
        if (NULL == xmetriccollectorinfo)
        {
            xmetriccollectorinfo = ripple_xmanager_metriccollectorinfo_init();
            rmemcpy1(xmetriccollectorinfo->pumpname, 0, pumpmane, len);
            pxmetriccollectornode->collectorinfo = dlist_put(pxmetriccollectornode->collectorinfo, xmetriccollectorinfo);
        }

        /* 设置 recvlsn */
        rmemcpy1(&xmetriccollectorinfo->recvlsn, 0, uptr, 8);
        xmetriccollectorinfo->recvlsn = r_ntoh64(xmetriccollectorinfo->recvlsn);
        uptr += 8;

        /* 获取 flushlsn */
        rmemcpy1(&xmetriccollectorinfo->flushlsn, 0, uptr, 8);
        xmetriccollectorinfo->flushlsn = r_ntoh64(xmetriccollectorinfo->flushlsn);
        uptr += 8;

        /* 获取 recvtrailno */
        rmemcpy1(&xmetriccollectorinfo->recvtrailno, 0, uptr, 8);
        xmetriccollectorinfo->recvtrailno = r_ntoh64(xmetriccollectorinfo->recvtrailno);
        uptr += 8;

        /* 获取 recvtrailstart */
        rmemcpy1(&xmetriccollectorinfo->recvtrailstart, 0, uptr, 8);
        xmetriccollectorinfo->recvtrailstart = r_ntoh64(xmetriccollectorinfo->recvtrailstart);
        uptr += 8;

        /* 获取 flushtrailno */
        rmemcpy1(&xmetriccollectorinfo->flushtrailno, 0, uptr, 8);
        xmetriccollectorinfo->flushtrailno = r_ntoh64(xmetriccollectorinfo->flushtrailno);
        uptr += 8;

        /* 获取 flushtrailstart */
        rmemcpy1(&xmetriccollectorinfo->flushtrailstart, 0, uptr, 8);
        xmetriccollectorinfo->flushtrailstart = r_ntoh64(xmetriccollectorinfo->flushtrailstart);
        uptr += 8;

        /* 获取 recvtimestamp */
        rmemcpy1(&xmetriccollectorinfo->recvtimestamp, 0, uptr, 8);
        xmetriccollectorinfo->recvtimestamp = r_ntoh64(xmetriccollectorinfo->recvtimestamp);
        uptr += 8;

        /* 获取 parsetimestamp */
        rmemcpy1(&xmetriccollectorinfo->flushtimestamp, 0, uptr, 8);
        xmetriccollectorinfo->flushtimestamp = r_ntoh64(xmetriccollectorinfo->flushtimestamp);
        uptr += 8;
    }
...
static ripple_xmanager_metricmsgop m_metricmsgops[] =
{
    ...
    },
    {
        ...
        "XManager Msg Pump Increment",
        NULL,
        ripple_xmanager_metricmsg_parsepumpincmsg
    },
    {
        RIPPLE_XMANAGER_MSG_PUMPONLINEREFRESH,
        "XManager Msg Pump Online Refresh",
        NULL,
        NULL
    },
    {
        RIPPLE_XMANAGER_MSG_PUMPBIGTXN,
        "XManager Msg Pump Big Transaction",
        NULL,
        NULL
    },
    {
        RIPPLE_XMANAGER_MSG_COLLECTORINCREMENT,
        "XManager Msg Collector Increment",
        NULL,
        ripple_xmanager_metricmsg_parsecollectorincmsg
    },
    {
        RIPPLE_XMANAGER_MSG_COLLECTORONLINEREFRESH,
        "XManager Msg Collector Online Refresh",
        NULL,
        NULL
    },
    {
        RIPPLE_XMANAGER_MSG_COLLECTORBIGTXN,
        "XManager Msg Collector Big Transaction",
        NULL,
        NULL
    },
    {
