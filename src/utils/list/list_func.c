 ...
 *
 * list_func.c
 *      implementation for generic linked list package
 *
 *
...
check_list_invariants(const List *list)
{
	...
	Assert(list->type == TL_List ||
		   list->type == TL_IntList ||
		   list->type == TL_OidList);
 ...
 */
static List *
new_list(ListTag type)
{
	List	   *new_list;
	ListCell   *new_head;

	new_head = (ListCell *) LIST_MEM_ALLOC(sizeof(*new_head));
	new_head->next = NULL;
	/* new_head->data is left undefined! */

	new_list = (List *) LIST_MEM_ALLOC(sizeof(*new_list));
	new_list->type = type;
	new_list->length = 1;
...
new_tail_cell(List *list)
{
	...
	new_tail = (ListCell *) LIST_MEM_ALLOC(sizeof(*new_tail));
	new_tail->next = NULL;
...
add_new_cell(List *list, ListCell *prev_cell)
{
	...
	new_cell = (ListCell *) LIST_MEM_ALLOC(sizeof(*new_cell));
...
list_concat(List *list1, List *list2)
{
	...
	if (list1 == list2)
	{
		elog(RLOG_ERROR, "cannot list_concat() a list to itself");
	}
...
list_member(const List *list, const void *datum)
{
	...
	foreach(cell, list)
	{
		if (simple_equal(lfirst(cell), datum))
			return true;
	}
...
list_copy(const List *oldlist)
{
	...
	while (oldlist_cur)
	{
		...
		newlist_cur = (ListCell *) LIST_MEM_ALLOC(sizeof(*newlist_cur));
...
list_qsort(const List *list, list_qsort_comparator cmp)
{
	...
	list_arr = (ListCell **) LIST_MEM_ALLOC(sizeof(ListCell *) * len);
	i = 0;
	foreach(cell, list)
		list_arr[i++] = cell;
...
/* we may not use list with datum, so just compare simple */
static bool
simple_equal(const void *a, const void *b)
{
	bool		retval = false;
