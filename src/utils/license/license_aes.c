...
{
  KeyExpansion(ctx->RoundKey, key);
  rmemcpy1(ctx->Iv, 0, iv, AES_BLOCKLEN);
}

#if 0
static void AES_ctx_set_iv(struct AES_ctx* ctx, const uint8_t* iv)
{
  rmemcpy1(ctx->Iv, iv, AES_BLOCKLEN);
}
#endif
...
static void AES_CBC_decrypt_buffer(struct AES_ctx* ctx, uint8_t* buf,  uint32_t length)
{
  ...
  for (i = 0; i < length; i += AES_BLOCKLEN)
  {
    rmemcpy1(storeNextIv, 0, buf, AES_BLOCKLEN);
    InvCipher((state_t*)buf, ctx->RoundKey);
    XorWithIv(buf, ctx->Iv);
    rmemcpy1(ctx->Iv, 0, storeNextIv, AES_BLOCKLEN);
    buf += AES_BLOCKLEN;
  }
}

void ripple_license_decrypt(ripple_license_ctx *lctx, char *decrypt_data, size_t len)
{
    uint8_t *key = lctx->pkey;
    uint8_t *iv = lctx->iv;

    struct AES_ctx ctx;

    AES_init_ctx_iv(&ctx, key, iv);
    AES_CBC_decrypt_buffer(&ctx, (uint8_t *)decrypt_data, len);
}
