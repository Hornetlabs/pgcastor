           ...
           unsigned char output[32], int is224 )
{
    ...
    rmemset1( &ctx, 0, 0, sizeof( sha2_context ) );
}
                       ...
                       int is224 )
{
    ...
    rmemset1( ctx->ipad, 0, 0x36, 64 );
    rmemset1( ctx->opad, 0, 0x5C, 64 );
...
static void sha2_hmac_finish( sha2_context *ctx, unsigned char output[32] )
{
    ...
    rmemset1( tmpbuf, 0, 0, sizeof( tmpbuf ) );
}
    ...
    unsigned char *output)
{
    ...
    while (generated_key_length < key_length) {
        ...
        rmemcpy1(output + generated_key_length, 0, work, bytes_to_write);
        generated_key_length += bytes_to_write;
        ++counter;
    }
}

#define RIPPLE_PBKDF2_SALT_LEN          20
#define RIPPLE_PBKDF2_ITERATION_COUNT   4096
#define RIPPLE_PBKDF2_KEY_LEN           16

void ripple_license_get_pbkdf2_key(ripple_license_ctx *ctx)
{
    PKCS5_PBKDF2_HMAC((unsigned char *)ctx->password,
                      strlen(ctx->password),
                      ctx->salt,
                      RIPPLE_PBKDF2_SALT_LEN,
                      RIPPLE_PBKDF2_ITERATION_COUNT,
                      RIPPLE_PBKDF2_KEY_LEN,
                      ctx->pkey);
}
