 ...
 *   2、中转线程将 commit 后的数据进行落盘或转移至 ripple_queuecache 中
*/
...
ripple_stmtcache_add_retry:
    /* 加入到链表中 */
    iret = ripple_thread_lock(&stmtcache->mutex_lock);
    if(0 != iret)
    {
    ...
    if((stmtcache->totalnum + 1) > g_transmaxnum)
    {
        ...
        if(true == stmtcache->wsignal)
        {
            ...
            ripple_thread_cond_signal(&stmtcache->cond);
        }
        ripple_thread_unlock(&stmtcache->mutex_lock);
        usleep(10000);
        elog(RLOG_DEBUG, "stmtcache full,waiting space, totalnum:%d, maxnum:%d",
                            stmtcache->totalnum,
                            g_transmaxnum);
        goto ripple_stmtcache_add_retry;
    }
    ...
    *timeout = 0;
    while(1)
    {
        ...
        if(NULL == stmtcache->head)
        {
            ...
            iret = ripple_thread_cond_timewait(&stmtcache->cond, &stmtcache->mutex_lock, &ts);
            if(0 != iret)
            {
                if(iret == ETIMEDOUT)
                {
        ...
        /* 解锁 */
        iret = ripple_thread_unlock(&stmtcache->mutex_lock);
        if(0 != iret)
        {
...
void ripple_cache_txn_destroy(ripple_cache_txn* stmtcache)
{
    ...
    ripple_thread_mutex_destroy(&stmtcache->mutex_lock);
    ripple_thread_cond_destroy(&stmtcache->cond);
...
{
    ripple_cache_txn* stmtcache = NULL;
    stmtcache = (ripple_cache_txn*)rmalloc1(sizeof(ripple_cache_txn));
    if(NULL == stmtcache)
    {
        elog(RLOG_ERROR, "out of memory");
    }
    rmemset0(stmtcache, 0, '\0', sizeof(ripple_cache_txn));
    ...
    /* 锁和信号初始化 */
    ripple_thread_mutex_init(&stmtcache->mutex_lock, NULL);

    if(0 != ripple_thread_cond_init(&stmtcache->cond, NULL))
    {
