...
void misc_lockfiles_create(const char *filename)
{
    ...
    for (ntries = 0;; ntries++)
    {
        ...
        */
        ...
        if (len == 0)
        {
            elog(RLOG_ERROR, "lock file %s is empty, Either another ripple is starting, or the lock file is the remnant of a previous server startup crash.", filename);
        }
            ...
            && other_pid != my_p_pid)
        {
            ...
            {
                elog(RLOG_ERROR, "lock file %s already exists, Is another ripple (PID %d) running in data directory:%s",
        ...
        */
        ...
        {
            elog(RLOG_ERROR, "could not remove old lock file %s, "
