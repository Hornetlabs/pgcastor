...
void enum_getfromdb(PGconn* conn, cache_sysdicts* sysdicts)
{
    ...
    for (i = 0; i < PQntuples(res); i++)
    {
        ...
        entry = (catalog_enum_value*)hash_search(sysdicts->by_enum, &enum_obj->enumtypid, HASH_ENTER, &found);
        if (!found)
        {
            entry->enums = NIL;
        }
...
void enum_catalogdata2transcache(cache_sysdicts* sysdicts, catalogdata* catalogdata)
{
    ...
    else if (CATALOG_OP_UPDATE == catalogdata->op)
    {
        ...
        {
            elog(RLOG_WARNING, "pgenum %u, %s can not fond in enum hash", pgenum->enumtypid, pgenum->enumlabel.data);
            return;
        }
        ...
        elog(RLOG_WARNING, "enum %u, %s can not fond in enum hash", pgenum->enumtypid, pgenum->enumlabel.data);
