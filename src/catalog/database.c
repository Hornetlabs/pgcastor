...
void database_getfromdb(PGconn* conn, cache_sysdicts* sysdicts)
{
    ...
    const char*                 query =
        "SELECT rel.oid, rel.datname, rel.datdba, rel.encoding, rel.datcollate, rel.datctype FROM "
        "pg_database rel;";
...
HTAB* databasecache_load(sysdict_header_array* array)
{
    ...
    hash_ctl.entrysize = sizeof(catalog_database_value);
    databasehtab = hash_create("catalog_database_value", 2048, &hash_ctl, HASH_ELEM | HASH_BLOBS);
