...
void proc_getfromdb(PGconn* conn, cache_sysdicts* sysdicts)
{
    ...
    const char*             query =
        "SELECT rel.oid,rel.proname, rel.pronamespace,rel.pronargs, rel.proargtypes FROM pg_proc "
        "rel;";
...
HTAB* proccache_load(sysdict_header_array* array)
{
    ...
    hash_ctl.entrysize = sizeof(catalog_proc_value);
    prochtab = hash_create("catalog_proc_value", 2048, &hash_ctl, HASH_ELEM | HASH_BLOBS);
