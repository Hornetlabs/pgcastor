...
#define pg_parser_SizeOfMultiInsertTuple (offsetof(pg_parser_xl_multi_insert_tuple, t_hoff) \
                                            + sizeof(uint8_t))
...
#define pg_parser_SizeOfHeapInsert (offsetof(pg_parser_xl_heap_insert, flags) \
                                      + sizeof(uint8_t))

#define pg_parser_SizeOfHeapDelete (offsetof(pg_parser_xl_heap_delete, flags) \
                                      + sizeof(uint8_t))

#define pg_parser_SizeOfHeapUpdate (offsetof(pg_parser_xl_heap_update, new_offnum) \
                                      + sizeof(uint16_t))

extern bool pg_parser_trans_rmgr_heap2_pre(
                            pg_parser_trans_transrec_decode_XLogReaderState *state,
                            pg_parser_translog_pre_base **result, 
                            int32_t *pg_parser_errno);

extern bool pg_parser_trans_rmgr_heap_pre(
                            pg_parser_trans_transrec_decode_XLogReaderState *state,
                            pg_parser_translog_pre_base **result, 
                            int32_t *pg_parser_errno);
...
extern bool pg_parser_trans_rmgr_heap_trans_get_tuple(pg_parser_trans_transrec_decode_XLogReaderState *state,
                            pg_parser_translog_tbcolbase **result,
                            int32_t *pg_parser_errno);

extern bool pg_parser_trans_rmgr_heap2_trans_get_tuple(pg_parser_trans_transrec_decode_XLogReaderState *state,
                            pg_parser_translog_tbcolbase **result,
                            int32_t *pg_parser_errno);
