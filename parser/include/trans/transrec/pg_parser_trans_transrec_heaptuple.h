    ...
    (pg_parser_AssertMacro((attnum) > 0),                                                        \
     ...
     pg_parser_HeapTupleNoNulls(tup)                                                             \
         ? (pg_parser_TupleDescAttr((tupleDesc), (attnum) - 1)->attcacheoff >= 0                 \
                ? (pg_parser_fetchatt(                                                           \
                      pg_parser_TupleDescAttr((tupleDesc), (attnum) - 1),                        \
...
#define PG_PARSER_MINIMAL_TUPLE_OFFSET                                           \
    ((offsetof(pg_parser_HeapTupleHeaderData, t_infomask2) - sizeof(uint32_t)) / \
     PG_PARSER_MAXIMUM_ALIGNOF * PG_PARSER_MAXIMUM_ALIGNOF)
