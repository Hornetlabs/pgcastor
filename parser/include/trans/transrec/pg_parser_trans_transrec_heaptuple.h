...
#define PG_PARSER_HEAP_KEYS_UPDATED          0x2000    /* tuple was updated and key cols
...
#define pg_parser_SizeOfHeapHeader (offsetof(pg_parser_xl_heap_header, t_hoff) \
                                     + sizeof(uint8_t))
...
#define pg_parser_ReorderBufferTupleBufData(p) \
    ((pg_parser_HeapTupleHeader) PG_PARSER_MAXALIGN(((char *) p) \
...
#define pg_parser_HeapTupleHeaderSetXmin(tup, xid) \
( \
    (tup)->t_choice.t_heap.t_xmin = (xid) \
)
#define pg_parser_HeapTupleHeaderSetXmax(tup, xid) \
( \
...
( \
    (attbyval) ? \
    ( \
        (attlen) == (int32_t) sizeof(pg_parser_Datum) ? \
            *((pg_parser_Datum *)(T)) \
        : \
      ( \
        ...
        ( \
            (attlen) == (int32_t) sizeof(int16_t) ? \
                pg_parser_Int16GetDatum(*((int16_t *)(T))) \
            : \
            ( \
...
#define PG_PARSER_VARATT_NOT_PAD_BYTE(PTR) \
    (*((uint8_t *) (PTR)) != 0)

#define pg_parser_att_align_pointer(cur_offset, attalign, attlen, attptr) \
( \
    ((attlen) == -1 && PG_PARSER_VARATT_NOT_PAD_BYTE(attptr)) ? \
...
( \
    ...
    : ((attlen == -1) ? \
    ( \
        (cur_offset) + (int32_t)PG_PARSER_VARSIZE_ANY(attptr) \
    ) \
    : \
    ( \
...
#define pg_parser_HeapTupleHasNulls(tuple) \
        (((tuple)->t_data->t_infomask & PG_PARSER_HEAP_HASNULL) != 0)
#define pg_parser_HeapTupleNoNulls(tuple) \
        (!((tuple)->t_data->t_infomask & PG_PARSER_HEAP_HASNULL))
#define pg_parser_HeapTupleHasVarWidth(tuple) \
...
#define pg_parser_HeapTupleHeaderGetNatts(tup) \
    ((tup)->t_infomask2 & PG_PARSER_HEAP_NATTS_MASK)
...
#define pg_parser_HeapTupleHeaderGetRawXmin(tup) \
( \
    (tup)->t_choice.t_heap.t_xmin \
)

#define pg_parser_HeapTupleHeaderGetRawXmax(tup) \
( \
    (tup)->t_choice.t_heap.t_xmax \
)

#define pg_parser_HeapTupleHeaderGetRawCommandId(tup) \
( \
    (tup)->t_choice.t_heap.t_field3.t_cid \
)

#define PG_PARSER_MINIMAL_TUPLE_OFFSET \
    ((offsetof(pg_parser_HeapTupleHeaderData, t_infomask2) - sizeof(uint32_t)) / PG_PARSER_MAXIMUM_ALIGNOF * PG_PARSER_MAXIMUM_ALIGNOF)
...
#define PG_PARSER_MINIMAL_TUPLE_DATA_OFFSET \
    offsetof(pg_parser_MinimalTupleData, t_infomask2)
