...
#define PG_PARSER_TRANS_TRANSREC_DECODE_HY
...
#define PG_PARSER_TRANS_TRANSREC_BKPBLOCK_HAS_IMAGE    0x10    /* block data is an XLogRecordBlockImage */
...
#define PG_PARSER_TRANS_TRANSREC_BKPIMAGE_APPLY        0x04    /* page image should be restored during
                                     * replay */

#define pg_parser_XLogRecHasBlockImage(decoder, block_id) \
...
#define pg_parser_ItemIdHasStorage(itemId) \
    ((itemId)->lp_len != 0)

#define pg_parser_ItemIdGetOffset(itemId) \
   ((itemId)->lp_off)
...
typedef struct pg_parser_XLogRecord
{
    ...
    pg_parser_XLogRecPtr           l_prev;            /* 上一条record的偏移 */
    uint8_t              xl_info;           /* 标志位 */
 ...
 */
struct pg_parser_trans_transrec_decode_XLogReaderState
{
...
typedef struct pg_parser_trans_transrec_decode_XLogReaderState
               pg_parser_trans_transrec_decode_XLogReaderState;

/* 函数声明 begin*/
extern pg_parser_trans_transrec_decode_XLogReaderState 
...
extern bool pg_parser_trans_transrec_decodeXlogRecord(
                                pg_parser_trans_transrec_decode_XLogReaderState *state,
...
extern char *pg_parser_XLogRecGetBlockData(
                            pg_parser_trans_transrec_decode_XLogReaderState *record,
                            uint8_t block_id,
                            size_t *len);
/* 函数声明 end */
