...
#define pg_parser_BlockIdIsValid(blockId) \
    ((bool) pg_parser_PointerIsValid(blockId))
...
#define pg_parser_ItemPointerGetOffsetNumberNoCheck(pointer) \
( \
    (pointer)->ip_posid \
)
...
#define pg_parser_ItemIdIsUsed(itemId) \
    ((itemId)->lp_flags != PG_PARSER_LP_UNUSED)

#define pg_parser_ItemIdIsRedirected(itemId) \
    ((itemId)->lp_flags == PG_PARSER_LP_REDIRECT)

#define pg_parser_ItemIdIsNormal(itemId) \
    ((itemId)->lp_flags == PG_PARSER_LP_NORMAL)
