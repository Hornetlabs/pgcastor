...
typedef struct PG_PARSER_TRANS_RMGR_HEAP
{
    ...
    pg_parser_trans_transrec_rmgr_info_func_trans     m_infofunc_trans;     /* 二次解析时info级的处理函数 */
...
/* static statement */
static bool pg_parser_trans_rmgr_heap_insert_trans(pg_parser_trans_transrec_decode_XLogReaderState *state,
                                                        pg_parser_translog_tbcolbase **result,
                                                        int32_t *pg_parser_errno);

static bool pg_parser_trans_rmgr_heap2_minsert_trans(pg_parser_trans_transrec_decode_XLogReaderState *state,
                                                        pg_parser_translog_tbcolbase **result,
                                                        int32_t *pg_parser_errno);

static bool pg_parser_trans_rmgr_heap_delete_trans(pg_parser_trans_transrec_decode_XLogReaderState *state,
                                                        pg_parser_translog_tbcolbase **result,
                                                        int32_t *pg_parser_errno);

static bool pg_parser_trans_rmgr_heap_update_trans(pg_parser_trans_transrec_decode_XLogReaderState *state,
                                                        pg_parser_translog_tbcolbase **result,
                                                        int32_t *pg_parser_errno);

static bool pg_parser_trans_rmgr_heap_insert_get_tuple(pg_parser_trans_transrec_decode_XLogReaderState *state,
                                                        pg_parser_translog_tbcolbase **result,
                                                        int32_t *pg_parser_errno);

static bool pg_parser_trans_rmgr_heap_delete_get_tuple(pg_parser_trans_transrec_decode_XLogReaderState *state,
                                                        pg_parser_translog_tbcolbase **result,
                                                        int32_t *pg_parser_errno);

static bool pg_parser_trans_rmgr_heap2_minsert_get_tuple(pg_parser_trans_transrec_decode_XLogReaderState *state,
                                                        pg_parser_translog_tbcolbase **result,
                                                        int32_t *pg_parser_errno);

static bool pg_parser_trans_rmgr_heap_update_get_tuple(pg_parser_trans_transrec_decode_XLogReaderState *state,
                                                        pg_parser_translog_tbcolbase **result,
                                                        int32_t *pg_parser_errno);
...
bool pg_parser_trans_rmgr_heap_trans_get_tuple(pg_parser_trans_transrec_decode_XLogReaderState *state,
                            pg_parser_translog_tbcolbase **result,
                            int32_t *pg_parser_errno)
{
    uint8_t info = state->decoded_record->xl_info;
...
bool pg_parser_trans_rmgr_heap2_trans_get_tuple(pg_parser_trans_transrec_decode_XLogReaderState *state,
                            pg_parser_translog_tbcolbase **result,
                            int32_t *pg_parser_errno)
{
    uint8_t info = state->decoded_record->xl_info;
                                                        ...
                                                        int32_t *pg_parser_errno)
{
    ...
    if(!pg_parser_sysdict_getTableInfo(relfilenode,
    ...
    if (true)
    {
        ...
        if (pg_parser_XLogRecHasBlockImage(state, 0))
        {
            ...
            /* 组装完整page */
            if (!pg_parser_image_get_block_image(state,
                                                    0,
                                                    page,
                                                    state->trans_data->m_pagesize))
            {
    ...
    for (natt = 0; natt < tupdesc->natts; natt++)
    {
        ...
        origval = pg_parser_heap_getattr(&tuple->tuple,
                                             natt + 1,
                                             tupdesc,
                                             &isnull,
                                             &ismissing,
        ...
        else if (!pg_parser_convert_attr_to_str_value(origval,
                                                         state->trans_data->m_sysdicts,
                                                         colvalue,
                                                         &zicinfo))
        {
                                                        ...
                                                        int32_t *pg_parser_errno)
{
    ...
    for (i = 0; i < minsert_record->m_rowCnt; i++)
    {
        ...
        for (natt = 0; natt < tupdesc->natts; natt++)
        {
            ...
            origval = pg_parser_heap_getattr(&tuple[i]->tuple,
                                                natt + 1,
                                                tupdesc,
                                                &isnull,
                                                &ismissing,
                                                        ...
                                                        int32_t *pg_parser_errno)
{
    ...
    else
    {
        ...
        pg_parser_DecodeXLogTuple((char *) xlrec + pg_parser_SizeOfHeapDelete,
    ...
    for (natt = 0; natt < tupdesc->natts; natt++)
    {
        ...
        zicinfo.errorno = pg_parser_errno;
        zicinfo.debuglevel = state->trans_data->m_debugLevel;
        if (!pg_parser_convert_attr_to_str_value(origval,
                                                    state->trans_data->m_sysdicts,
                                                    colvalue,
                                                    &zicinfo))
        {
                                                        ...
                                                        int32_t *pg_parser_errno)
{
    ...
    else
    {
        ...
        have_old_tuple_data = ((xlrec->flags & PG_PARSER_TRANS_XLH_UPDATE_CONTAINS_OLD)
        ...
        if (have_old_tuple_data)
        {
            ...
            pg_parser_DecodeXLogTuple((char *) xlrec + pg_parser_SizeOfHeapUpdate,
    ...
    for (natt = 0; natt < tupdesc->natts; natt++)
    {
        ...
        origval = pg_parser_heap_getattr(&tuple_old->tuple,
                                                    natt + 1,
                                                    tupdesc,
                                                    &isnull,
                                                    &ismissing,
    ...
    for (natt = 0; natt < tupdesc->natts; natt++)
    {
        ...
        origval = pg_parser_heap_getattr(&tuple_new->tuple,
                                                    natt + 1,
                                                    tupdesc,
                                                    &isnull,
                                                    &ismissing,
                            ...
                            int16_t dbtype)
{
    ...
    for (blcknum = 0; blcknum < readstate->max_block_id + 1; blcknum++)
    {
        ...
        /* 组装完整page */
        if (!pg_parser_image_get_block_image(readstate,
                                                blcknum,
                                                page,
                                                readstate->pre_trans_data->m_pagesize))
        {
                                              ...
                                              char *dbversion)
{
    ...
    pg_parser_HeapTupleHeader htup;             /* 组装新 tuple 时，便于 coding 的指针。 */
