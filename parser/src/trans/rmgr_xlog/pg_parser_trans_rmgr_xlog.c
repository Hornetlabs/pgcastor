...
static bool pg_parser_trans_rmgr_xlog_ckps(
                            pg_parser_trans_transrec_decode_XLogReaderState *state,
                            pg_parser_translog_pre_base **result,
                            int32_t *pg_parser_errno);

static bool pg_parser_trans_rmgr_xlog_ckpo(
                            pg_parser_trans_transrec_decode_XLogReaderState *state,
                            pg_parser_translog_pre_base **result,
                            int32_t *pg_parser_errno);

static bool pg_parser_trans_rmgr_xlog_switch(
                            pg_parser_trans_transrec_decode_XLogReaderState *state,
                            pg_parser_translog_pre_base **result,
                            int32_t *pg_parser_errno);
...
{
    pg_parser_trans_rmgr_xlog_info                   m_infoid;       /* info值 */
                            ...
                            int32_t *pg_parser_errno)
{
    ...
    if (!pg_parser_mcxt_malloc(RMGR_XLOG_MCXT,
                                 (void **) (&transchp),
