...
static bool pg_parser_trans_transrec_decode_checkPreTransParam(pg_parser_translog_pre
                                                                 *pg_parser_pre_data);
...
{
    pg_parser_trans_rmgr_enum                 m_rmgrid;           /* rmgrid值 */
    pg_parser_trans_transrec_rmgrfunc_pre     m_rmgrfunc_pre;     /* 预解析接口rmgr级的处理函数 */
    pg_parser_trans_transrec_rmgrfunc_trans   m_rmgrfunc_trans;    /* 二次解析时函数处理接口 */
...
static bool deal_invalid_record(pg_parser_trans_transrec_decode_XLogReaderState *readstate,
                                ...
                                int32_t *pg_parser_errno)
{
    if (!pg_parser_mcxt_malloc(PARSER_MCXT,
                                 (void **) (pg_parser_result),
...
{
    pg_parser_trans_transrec_decode_XLogReaderState *readstate = NULL;
    pg_parser_XLogRecord      *record = (pg_parser_XLogRecord *)
                                            pg_parser_transData->m_record;
    bool    result = false;
    int32_t index = 0;
    int8_t  rmgrcnts = PG_PARSER_RMGRCNTS;
...
void pg_parser_trans_preTrans_free(pg_parser_translog_pre_base *pg_parser_result)
{
    if (pg_parser_result)
    {
        ...
        {
            pg_parser_translog_pre_trans *trans = (pg_parser_translog_pre_trans *)
                                                      pg_parser_result;
            pg_parser_xl_xact_parsed_commit *commit = (pg_parser_xl_xact_parsed_commit *)
                                                          trans->m_transdata;
            if (commit)
            {
                if (commit->subxacts)
         ...
         || PG_PARSER_TRANSLOG_XACT_ABORT_PREPARE == (pg_parser_result)->m_type)
        {
            pg_parser_translog_pre_trans *trans = (pg_parser_translog_pre_trans *)
                                                      pg_parser_result;
            pg_parser_xl_xact_parsed_abort *abort = (pg_parser_xl_xact_parsed_abort *)
                                                          trans->m_transdata;
        ...
        {
            int32_t i = 0;
            pg_parser_translog_pre_image_tuple *image = (pg_parser_translog_pre_image_tuple *)
                                                            pg_parser_result;
        ...
        {
            pg_parser_translog_pre_relmap *relmap = (pg_parser_translog_pre_relmap *)
                                                            pg_parser_result;
        ...
        {
            pg_parser_translog_pre_running_xact *rxact = (pg_parser_translog_pre_running_xact *)
                                                             pg_parser_result;
            if (rxact)
            {
                if (rxact->m_standby)
        ...
        {
            pg_parser_translog_pre_heap_truncate *truncate = (pg_parser_translog_pre_heap_truncate *)
                                                                 pg_parser_result;
            if (truncate)
            {
                if (truncate->relids)
...
void pg_parser_trans_ddl_free(pg_parser_translog_systb2ddl *ddl,
        ...
        if (pg_parser_trans_pre->m_sysdicts)
        {
            if (pg_parser_trans_pre->m_sysdicts->m_pg_attribute.m_pg_attributes)
                pg_parser_mcxt_free(PARSER_MCXT, 
                    pg_parser_trans_pre->m_sysdicts->m_pg_attribute.m_pg_attributes);
