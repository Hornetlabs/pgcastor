...
static pg_parser_translog_ddlstmt* pg_parser_ddl_assemble_create_view(pg_parser_translog_systb2ddl* pg_parser_ddl,
                                                                      pg_parser_ddlstate*           ddlstate,
                                                                      int32_t*                      pg_parser_errno);

pg_parser_translog_ddlstmt* pg_parser_DDL_create_view(pg_parser_translog_systb2ddl*        pg_parser_ddl,
    ...
    if (IS_INSERT(current_record->m_record))
    {
                                            ...
                                            pg_parser_ddl->m_dbversion))
        {
            ...
            temp_classid = PG_PARSER_DDL_GETCOLUMNVALUEBYNAME("classid",
                                                              ...
                                                              current_record->m_record->m_valueCnt,
                                                              temp_classid);
            if (!strcmp(ddlstate->m_reloid_char, temp_refobjid) && !strcmp(RewriteRelationIdChar, temp_classid))
            {
                result = pg_parser_ddl_assemble_create_view(pg_parser_ddl, ddlstate, pg_parser_errno);
            }
        }
    }
    return result;
                                                                      ...
                                                                      int32_t*                      pg_parser_errno)
{
    ...
    pg_parser_log_errlog(pg_parser_ddl->m_debugLevel, "DEBUG, DDL PARSER: create view(ignore) end \n");
