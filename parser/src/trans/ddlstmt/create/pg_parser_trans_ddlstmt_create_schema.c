...
#define DDL_CREATE_SCHEMA_MCXT NULL

pg_parser_translog_ddlstmt* pg_parser_DDL_create_schema(pg_parser_translog_systb2ddl*        pg_parser_ddl,
                                                        pg_parser_translog_systb2dll_record* current_record,
                                                        pg_parser_ddlstate*                  ddlstate,
                                                        int32_t*                             pg_parser_errno)
{
    pg_parser_translog_ddlstmt*           result = NULL;
    pg_parser_translog_ddlstmt_valuebase* creatensp = NULL;
    pg_parser_log_errlog(pg_parser_ddl->m_debugLevel, "DEBUG, DDL PARSER: capture create schema end \n");
    PG_PARSER_UNUSED(pg_parser_ddl);
    PG_PARSER_UNUSED(current_record);
    PG_PARSER_UNUSED(pg_parser_errno);

    if (!pg_parser_mcxt_malloc(DDL_CREATE_SCHEMA_MCXT, (void**)&result, sizeof(pg_parser_translog_ddlstmt)))
    {
        *pg_parser_errno = ERRNO_PG_PARSER_DDL_MEMERR_ALLOC_26;
        return NULL;
    }
    if (!pg_parser_mcxt_malloc(DDL_CREATE_SCHEMA_MCXT,
                               (void**)&creatensp,
                               sizeof(pg_parser_translog_ddlstmt_valuebase)))
    {
        *pg_parser_errno = ERRNO_PG_PARSER_DDL_MEMERR_ALLOC_27;
        return NULL;
    }
    ...
    result->m_base.m_ddlinfo = PG_PARSER_DDLINFO_CREATE_NAMESPACE;
