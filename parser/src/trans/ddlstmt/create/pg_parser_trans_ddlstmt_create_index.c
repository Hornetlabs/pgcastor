                                                            ...
                                                            int32_t *pg_parser_errno)
{
    ...
    if (IS_INSERT(current_record->m_record))
    {
        ...
        else if (pg_parser_check_table_name(current_record->m_record->m_base.m_tbname, SYS_INDEX, pg_parser_ddl->m_dbtype, pg_parser_ddl->m_dbversion))
        {
            /* 存储pg_index的记录 */
            pg_parser_log_errlog(pg_parser_ddl->m_debugLevel, 
        ...
        else if (pg_parser_check_table_name(current_record->m_record->m_base.m_tbname, SYS_CONSTRAINT, pg_parser_ddl->m_dbtype, pg_parser_ddl->m_dbversion))
        {
                                                ...
                                                int32_t *pg_parser_errno)
{
        ...
        /* indexprs可能是空的 */
        Node = pg_parser_ddl_getColumnValueByName("indexprs",
                                        ddlstate->m_index->m_new_values,
                                        ddlstate->m_index->m_valueCnt);
        if (Node)
