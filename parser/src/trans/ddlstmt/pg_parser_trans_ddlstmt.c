...
static pg_parser_translog_ddlstmt* pg_parser_DDL_toast_index_drop_escape(pg_parser_translog_systb2ddl *pg_parser_ddl,
                                                            pg_parser_translog_systb2dll_record *current_record,
                                                            pg_parser_ddlstate *ddlstate,
                                                            int32_t *pg_parser_errno);
...
static pg_parser_DDL_kindWithFunc m_ddl_transfunc[] = 
{
    ...
    {PG_PARSER_DDL_TABLE_CREATE_PARTITION_SUB, pg_parser_DDL_create_table_partition_sub}, /* 含有pg_node_tree */
    ...
    {PG_PARSER_DDL_TABLE_ALTER_COLUMN_DEFAULT, pg_parser_DDL_alter_table_column_add_default}, /* 含有pg_node_tree */
    ...
    {PG_PARSER_DDL_VIEW_CREATE, pg_parser_DDL_create_view}, /* 含有pg_node_tree */
    ...
    while (current_record)
    {
        ...
        else
        {
            for (i = PG_PARSER_DDL_TRANSDDL_NUM_SHORT; i < PG_PARSER_DDL_TRANSDDL_NUM_MAX; i++)
            {
                ...
                {
                    current_result = m_ddl_transfunc[i].m_transfunc(pg_parser_ddl, current_record, &ddlstate, pg_parser_errno);
                    break;
                }
            }
        }
