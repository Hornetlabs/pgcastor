...
pg_parser_translog_ddlstmt* pg_parser_DDL_drop_schema(pg_parser_translog_systb2ddl*        pg_parser_ddl,
                                                      ...
                                                      int32_t*                             pg_parser_errno)
{
    ...
    pg_parser_log_errlog(pg_parser_ddl->m_debugLevel, "DEBUG, DDL PARSER: capture drop schema end \n");
    PG_PARSER_UNUSED(pg_parser_ddl);
    PG_PARSER_UNUSED(current_record);
    PG_PARSER_UNUSED(pg_parser_errno);

    if (!pg_parser_mcxt_malloc(DDL_DROP_SCHEMA_MCXT, (void**)&result, sizeof(pg_parser_translog_ddlstmt)))
    {
        *pg_parser_errno = ERRNO_PG_PARSER_DDL_MEMERR_ALLOC_41;
        return NULL;
    }
    if (!pg_parser_mcxt_malloc(DDL_DROP_SCHEMA_MCXT, (void**)&dropensp, sizeof(pg_parser_translog_ddlstmt_valuebase)))
    {
