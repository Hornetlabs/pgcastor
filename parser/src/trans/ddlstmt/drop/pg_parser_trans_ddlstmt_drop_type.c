    ...
    int32_t* pg_parser_errno)
{
    ...
    if (IS_DELETE(current_record->m_record))
    {
                                       ...
                                       pg_parser_ddl->m_dbtype, pg_parser_ddl->m_dbversion))
        {
            ...
            temp_oid =
                PG_PARSER_DDL_GETCOLUMNVALUEBYNAME("oid", ddlstate->m_type_item->m_old_values,
                                                   ddlstate->m_type_item->m_valueCnt, temp_oid);
        ...
        else if (pg_parser_check_table_name(current_record->m_record->m_base.m_tbname, SYS_TYPE,
                                            pg_parser_ddl->m_dbtype, pg_parser_ddl->m_dbversion))
        {
            char* temp_typtype = NULL;
            char* temp_oid = NULL;
            temp_typtype = PG_PARSER_DDL_GETCOLUMNVALUEBYNAME(
                "typtype", current_record->m_record->m_old_values,
                current_record->m_record->m_valueCnt, temp_typtype);

            temp_oid = PG_PARSER_DDL_GETCOLUMNVALUEBYNAME(
                "typrelid", current_record->m_record->m_old_values,
            ...
            if (ddlstate->m_type_savepoint != TYPE_SAVE_CLASS)
            {
                ...
                else if (temp_typtype[0] == 'e' || temp_typtype[0] == 'r' || temp_typtype[0] == 'd')
                {
        ...
        else if (pg_parser_check_table_name(current_record->m_record->m_base.m_tbname, SYS_CLASS,
                                            pg_parser_ddl->m_dbtype, pg_parser_ddl->m_dbversion))
        {
    ...
    int32_t* pg_parser_errno)
{
    ...
    if (!pg_parser_mcxt_malloc(DDL_DROP_TYPE_MCXT, (void**)&result,
    ...
    // todo free
    if (!pg_parser_mcxt_malloc(DDL_DROP_TYPE_MCXT, (void**)&type_return,
    ...
    type_return->m_name =
        PG_PARSER_DDL_GETCOLUMNVALUEBYNAME("typname", ddlstate->m_type_item->m_old_values,
                                           ddlstate->m_type_item->m_valueCnt, type_return->m_name);
    ...
    pg_parser_log_errlog(pg_parser_ddl->m_debugLevel, "DEBUG, DDL PARSER: drop type end \n");
    pg_parser_ddl_init_ddlstate(ddlstate);
    result->m_next = NULL;
    return result;
}
