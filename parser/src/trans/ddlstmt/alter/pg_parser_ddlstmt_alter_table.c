...
pg_parser_translog_ddlstmt* pg_parser_DDL_alter_table_renameTable(
    ...
    int32_t* pg_parser_errno)
{
    ...
    if (!pg_parser_mcxt_malloc(DDL_ALTERTABLE_MCXT, (void**)&result,
                               sizeof(pg_parser_translog_ddlstmt)))
    {
        *pg_parser_errno = ERRNO_PG_PARSER_DDL_MEMERR_ALLOC_0E;
        return NULL;
    }
    if (!pg_parser_mcxt_malloc(DDL_ALTERTABLE_MCXT, (void**)&rename,
    ...
    int32_t* pg_parser_errno)
{
    ...
    if (IS_INSERT(current_record->m_record))
    {
                                       ...
                                       pg_parser_ddl->m_dbtype, pg_parser_ddl->m_dbversion))
        {
            ...
            {
                result = pg_parser_ddl_assemble_alter_table_add_constraint(pg_parser_ddl, ddlstate,
                                                                           pg_parser_errno);
            }
        }
    }
    ...
    int32_t* pg_parser_errno)
{
    ...
    if (IS_UPDATE(current_record->m_record) &&
        (pg_parser_check_table_name(current_record->m_record->m_base.m_tbname, SYS_CLASS,
                                    pg_parser_ddl->m_dbtype, pg_parser_ddl->m_dbversion)))
    {
        bool change_attnum =
            pg_parser_ddl_checkChangeColumn("relnatts", current_record->m_record->m_new_values,
        ...
        if (change_attnum)
        {
            ...
            result = pg_parser_ddl_assemble_alter_table_add_column(pg_parser_ddl, ddlstate,
                                                                   pg_parser_errno);
            return result;
        }
    }
     ...
     * This is a safety mechanism to prevent parsing errors caused by unknown DDL statements
     * Because inserting data into pg_attribute table is too common, many DDLs have this operation
     * Therefore when encountering this situation, treat current record as first record and
     * reprocess DDL
     */
    pg_parser_log_errlog(pg_parser_ddl->m_debugLevel,
                         "WARNING: in pg_parser_DDL_alter_table_add_column ddl jump out\n");
    pg_parser_ddl_init_ddlstate(ddlstate);
    ...
    int32_t* pg_parser_errno)
{
                                    ...
                                    pg_parser_ddl->m_dbtype, pg_parser_ddl->m_dbversion)))
    {
        ...
        temp_check_relpersistence_change = pg_parser_ddl_checkChangeColumn(
            "relpersistence", current_record->m_record->m_new_values,
            current_record->m_record->m_old_values, current_record->m_record->m_valueCnt,
            pg_parser_errno);
        temp_check_relfilenode_change =
            pg_parser_ddl_checkChangeColumn("relfilenode", current_record->m_record->m_new_values,
                                            current_record->m_record->m_old_values,
                                            current_record->m_record->m_valueCnt, pg_parser_errno);
        if (temp_check_relpersistence_change && temp_check_relfilenode_change)
        {
            ...
            {
                pg_parser_log_errlog(pg_parser_ddl->m_debugLevel,
                                     "DEBUG, DDL PARSER: in alter table set unlogged/logged, step "
                                     "get class info \n");
                /* Set whether logged or unlogged is determined when getting pg_class */
                if (!pg_parser_ddl_get_pg_class_info(ddlstate, current_record->m_record, true))
                {
                    *pg_parser_errno = ERRNO_PG_PARSER_DDLSTMT_GET_CLASSINFO;
                    return NULL;
            ...
            else if (PG_PARSER_DDL_ALTER_LOG_GET_CLASS_UPDATE_RELPERSISTENCE_STEP ==
                     ddlstate->m_log_step)
            {
                /* Here only need to get oid of internal temporary table starting with pg_temp */
                char* temp_str = NULL;
                pg_parser_log_errlog(pg_parser_ddl->m_debugLevel,
                                     "DEBUG, DDL PARSER: in alter table set unlogged/logged, step "
                                     "get temp class \n");
    ...
    if (IS_DELETE(current_record->m_record))
    {
                                       ...
                                       pg_parser_ddl->m_dbtype, pg_parser_ddl->m_dbversion))
        {
            ...
            {
                char* temp_str = PG_PARSER_DDL_GETCOLUMNVALUEBYNAME(
                    "oid", current_record->m_record->m_old_values,
                    current_record->m_record->m_valueCnt, temp_str);
                if (strtoul(temp_str, NULL, 10) == ddlstate->m_reloid_temp)
                {
                    pg_parser_log_errlog(pg_parser_ddl->m_debugLevel,
                                         "DEBUG, DDL PARSER: in alter table set unlogged/logged, "
                                         "step delete temp class \n");
        ...
        {
            char* temp_str =
                PG_PARSER_DDL_GETCOLUMNVALUEBYNAME("objid", current_record->m_record->m_old_values,
                                                   current_record->m_record->m_valueCnt, temp_str);
            if (strtoul(temp_str, NULL, 10) == ddlstate->m_reloid_temp)
            {
                if (PG_PARSER_DDL_ALTER_LOG_GET_TEMP_CLASS_DELETE_STEP == ddlstate->m_log_step)
                {
                    result = pg_parser_ddl_assemble_alter_table_set_log(pg_parser_ddl, ddlstate,
                                                                        pg_parser_errno);
                }
            }
        }
    ...
    int32_t* pg_parser_errno)
{
    ...
    if (IS_DELETE(current_record->m_record))
    {
        /* primary key, unique capture records of pg_constraint */
        if (pg_parser_check_table_name(current_record->m_record->m_base.m_tbname, SYS_CONSTRAINT,
                                       pg_parser_ddl->m_dbtype, pg_parser_ddl->m_dbversion))
        {
            char* temp_str = NULL;
            ...
            ddlstate->m_relname = PG_PARSER_DDL_GETCOLUMNVALUEBYNAME(
                "conname", current_record->m_record->m_old_values,
                current_record->m_record->m_valueCnt, ddlstate->m_relname);
            ddlstate->m_reloid_char = PG_PARSER_DDL_GETCOLUMNVALUEBYNAME(
                "conrelid", current_record->m_record->m_old_values,
                current_record->m_record->m_valueCnt, ddlstate->m_reloid_char);
            temp_str = PG_PARSER_DDL_GETCOLUMNVALUEBYNAME(
            ...
            ddlstate->m_nspname_oid_char = PG_PARSER_DDL_GETCOLUMNVALUEBYNAME(
                ...
                current_record->m_record->m_valueCnt, ddlstate->m_nspname_oid_char);
                                            ...
                                            pg_parser_ddl->m_dbtype, pg_parser_ddl->m_dbversion))
        {
            char* temp_classid = PG_PARSER_DDL_GETCOLUMNVALUEBYNAME(
                "classid", current_record->m_record->m_old_values,
                current_record->m_record->m_valueCnt, temp_classid);
            if (!strcmp(ConstraintRelationIdChar, temp_classid))
            {
                result = pg_parser_ddl_assemble_alter_table_drop_constraint(pg_parser_ddl, ddlstate,
                                                                            pg_parser_errno);
            }
        }
    }
...
pg_parser_translog_ddlstmt* pg_parser_DDL_alter_table_namespace(
    pg_parser_translog_systb2ddl*        pg_parser_ddl,
    pg_parser_translog_systb2dll_record* current_record, pg_parser_ddlstate* ddlstate,
    int32_t* pg_parser_errno)
{
    ...
    if (!pg_parser_mcxt_malloc(DDL_ALTERTABLE_MCXT, (void**)&new_namespace,
                               sizeof(pg_parser_translog_ddlstmt_altertable)))
    {
    ...
    int32_t* pg_parser_errno)
{
    ...
    // todo free
    if (!pg_parser_mcxt_malloc(DDL_ALTERTABLE_MCXT, (void**)&cons_return,
    ...
    temp_str = PG_PARSER_DDL_GETCOLUMNVALUEBYNAME("conrelid", ddlstate->m_constraint->m_new_values,
                                                  ddlstate->m_constraint->m_valueCnt, temp_str);
    cons_return->m_relid = strtoul(temp_str, NULL, 10);
    cons_return->m_consname = PG_PARSER_DDL_GETCOLUMNVALUEBYNAME(
        "conname", ddlstate->m_constraint->m_new_values, ddlstate->m_constraint->m_valueCnt,
        cons_return->m_consname);
    temp_str =
        PG_PARSER_DDL_GETCOLUMNVALUEBYNAME("connamespace", ddlstate->m_constraint->m_new_values,
                                           ddlstate->m_constraint->m_valueCnt, temp_str);
    cons_return->m_consnspoid = strtoul(temp_str, NULL, 10);

    temp_contype =
        PG_PARSER_DDL_GETCOLUMNVALUEBYNAME("contype", ddlstate->m_constraint->m_new_values,
                                           ddlstate->m_constraint->m_valueCnt, temp_contype);
    if ('p' == temp_contype[0])
    {
        ...
        if (!pg_parser_mcxt_malloc(DDL_ALTERTABLE_MCXT, (void**)&pkey,
        ...
        if (!pg_parser_mcxt_malloc(DDL_ALTERTABLE_MCXT, (void**)&column,
                                   sizeof(pg_parser_translog_ddlstmt_col) * pkey->m_colcnt))
        {
            *pg_parser_errno = ERRNO_PG_PARSER_DDL_MEMERR_ALLOC_16;
            return NULL;
        }
        /* Append column type information stored in dlstate->m_ddlList to return value */
        i = 0;
        pg_parser_foreach(cell, ddlstate->m_attList)
        {
            ...
            column[i].m_colname = PG_PARSER_DDL_GETCOLUMNVALUEBYNAME(
                "attname", consatt->m_new_values, consatt->m_valueCnt, column[i].m_colname);

            temp_str = PG_PARSER_DDL_GETCOLUMNVALUEBYNAME("atttypid", consatt->m_new_values,
                                                          consatt->m_valueCnt, temp_str);
    ...
    else if ('f' == temp_contype[0])
    {
        ...
        temp_conkey =
            PG_PARSER_DDL_GETCOLUMNVALUEBYNAME("conkey", ddlstate->m_constraint->m_new_values,
                                               ddlstate->m_constraint->m_valueCnt, temp_conkey);
        temp_confkey =
            PG_PARSER_DDL_GETCOLUMNVALUEBYNAME("confkey", ddlstate->m_constraint->m_new_values,
                                               ddlstate->m_constraint->m_valueCnt, temp_confkey);

        if (!pg_parser_mcxt_malloc(DDL_ALTERTABLE_MCXT, (void**)&fkey,
        ...
        temp_str =
            PG_PARSER_DDL_GETCOLUMNVALUEBYNAME("confrelid", ddlstate->m_constraint->m_new_values,
                                               ddlstate->m_constraint->m_valueCnt, temp_str);
        fkey->m_consfkeyid = strtoul(temp_str, NULL, 10);

        cons_return->m_type = PG_PARSER_DDL_CONSTRAINT_FOREIGNKEY;
        /* Since there is no explicit column count record, we must first traverse to get column
         * count */
        ...
        cnt = 0;
        temp_cursor1 = temp_conkey;
        if (!pg_parser_mcxt_malloc(DDL_ALTERTABLE_MCXT, (void**)&fkey->m_concols_position,
                                   sizeof(int16_t) * fkey->m_colcnt) ||
            !pg_parser_mcxt_malloc(DDL_ALTERTABLE_MCXT, (void**)&fkey->m_fkeycols_position,
                                   sizeof(int16_t) * fkey->m_colcnt))
        {
    ...
    else if ('u' == temp_contype[0])
    {
        ...
        if (!pg_parser_mcxt_malloc(DDL_ALTERTABLE_MCXT, (void**)&ukey,
                                   sizeof(pg_parser_translog_ddlstmt_tbconstraint_key)))
        {
    ...
    else if ('c' == temp_contype[0])
    {
        ...
        if (!pg_parser_mcxt_malloc(DDL_ALTERTABLE_MCXT, (void**)&check,
                                   sizeof(pg_parser_translog_ddlstmt_tbconstraint_check)))
        {
            *pg_parser_errno = ERRNO_PG_PARSER_DDL_MEMERR_ALLOC_1B;
            return NULL;
        }
        Node = PG_PARSER_DDL_GETCOLUMNVALUEBYNAME("conbin", ddlstate->m_constraint->m_new_values,
                                                  ddlstate->m_constraint->m_valueCnt, Node);
    ...
    {
        /* Other constraints, do not capture, clean up and exit */
        pg_parser_ddl_init_ddlstate(ddlstate);
        return NULL;
    }
    ...
    int32_t* pg_parser_errno)
{
    ...
    if (!pg_parser_mcxt_malloc(DDL_ALTERTABLE_MCXT, (void**)&drop_constraint,
    ...
    int32_t* pg_parser_errno)
{
    ...
    if (!pg_parser_mcxt_malloc(DDL_ALTERTABLE_MCXT, (void**)&add_column_table,
                               sizeof(pg_parser_translog_ddlstmt_addcolumn)))
    {
        *pg_parser_errno = ERRNO_PG_PARSER_DDL_MEMERR_ALLOC_1F;
        return NULL;
    }
    if (!pg_parser_mcxt_malloc(DDL_ALTERTABLE_MCXT, (void**)&add_column,
    ...
    add_column->m_colname = PG_PARSER_DDL_GETCOLUMNVALUEBYNAME(
        "attname", consatt->m_new_values, consatt->m_valueCnt, add_column->m_colname);
    temp_str = PG_PARSER_DDL_GETCOLUMNVALUEBYNAME("atttypid", consatt->m_new_values,
                                                  consatt->m_valueCnt, temp_str);
    add_column->m_coltypid = strtoul(temp_str, NULL, 10);
    temp_notnull = PG_PARSER_DDL_GETCOLUMNVALUEBYNAME("attnotnull", consatt->m_new_values,
                                                      consatt->m_valueCnt, temp_notnull);
    if ('t' == temp_notnull[0])
    {
    ...
    temp_str = PG_PARSER_DDL_GETCOLUMNVALUEBYNAME("atttypmod", consatt->m_new_values,
                                                  consatt->m_valueCnt, temp_str);
    ...
    int32_t* pg_parser_errno)
{
    ...
    if (!pg_parser_mcxt_malloc(DDL_ALTERTABLE_MCXT, (void**)&set_log,
