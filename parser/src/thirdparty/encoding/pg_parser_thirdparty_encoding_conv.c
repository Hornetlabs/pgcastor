...
/*
 * Currently PG supported character sets in worst case have 4 to 1 growth after conversion
...
const int32_t pg_parser_fmgr_encoding_nbuiltins =
    (sizeof(pg_parser_fmgr_encoding_builtins) / sizeof(pg_parser_FmgrBuiltinEncoding));
...
static int32_t pg_parser_encoding_getEncodingNum(char* name)
{
    ...
    {
        if (!strcmp(enc2gettext_tbl[i].pg_name, name) || !strcmp(enc2gettext_tbl[i].stand_name, name))
        {
...
char* pg_parser_encoding_convert(char* src_str, bool* needfree, char* dest_encoding, char* src_encoding)
{
    pg_parser_FmgrBuiltinEncoding* res = NULL;
    char*                          result = NULL;
