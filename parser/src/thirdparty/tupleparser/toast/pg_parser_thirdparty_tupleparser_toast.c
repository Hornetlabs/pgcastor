...
struct pg_parser_varlena *pg_parser_detoast_datum(struct pg_parser_varlena *datum, bool *is_toast, bool *need_free, int dbtype, char *dbversion)
{
...
pg_parser_heap_tuple_fetch_attr(struct pg_parser_varlena *attr, bool *is_toast)
{
...
struct pg_parser_varlena *pg_parser_detoast_datum_packed(struct pg_parser_varlena *datum, bool *is_toast, bool *need_free, int dbtype, char *dbversion)
{
                                                           ...
                                                           bool *need_free)
{
    ...
    if (pg_parser_lz_decompress(PG_PARSER_TOAST_COMPRESS_RAWDATA(attr),
                        ...
                        PG_PARSER_TOAST_COMPRESS_RAWSIZE(attr), true) < 0)
        return NULL;
    *need_free = true;
    return result;
}
