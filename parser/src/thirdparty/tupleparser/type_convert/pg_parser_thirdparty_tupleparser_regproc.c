...
pg_parser_Datum regprocout(pg_parser_Datum attr, pg_parser_extraTypoutInfo* info)
{
    ...
    if (pg_parser_sysdict_getProcInfoByOid(proid, sysdicts, &procname, &nspname))
    {
        if (nspname)
        {
            if (!pg_parser_mcxt_malloc(PGFUNC_REGPROC_MCXT, (void**)&result, strlen(procname) + strlen(nspname) + 2))
            {
    ...
    else
    {
        ...
        if (!pg_parser_mcxt_malloc(PGFUNC_REGPROC_MCXT, (void**)&result, PG_PARSER_NAMEDATALEN))
        {
            return (pg_parser_Datum)0;
        }
