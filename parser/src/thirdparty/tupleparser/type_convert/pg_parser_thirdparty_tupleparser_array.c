...
#define ARR_DIMS(a) \
        ((int32_t *) (((char *) (a)) + sizeof(ArrayType)))
#define ARR_LBOUND(a) \
        ((int32_t *) (((char *) (a)) + sizeof(ArrayType) + \
                  sizeof(int32_t) * ARR_NDIM(a)))

#define ARR_NULLBITMAP(a) \
        (ARR_HASNULL(a) ? \
         (uint8_t *) (((char *) (a)) + sizeof(ArrayType) + \
                    2 * sizeof(int32_t) * ARR_NDIM(a)) \
         : (uint8_t *) NULL)
...
pg_parser_Datum array_out(pg_parser_Datum attr, pg_parser_extraTypoutInfo *info)
{
    ...
    if (is_toast)
    {
        /* 暂不支持行外存储的array解析, 因为后续解析需要系统表, 而行外存储解析接口没有传入系统表 */
        if (info != NULL)
