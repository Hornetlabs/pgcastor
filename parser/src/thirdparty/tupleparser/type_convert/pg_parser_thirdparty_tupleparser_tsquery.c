...
#define RESIZEBUF(inf, addsize)                                                            \
    ...
    {                                                                                      \
        ...
        (inf)->buflen *= 2;                                                                \
        pg_parser_mcxt_realloc(PGFUNC_TSQUERY_MCXT, (void**)&((inf)->buf), (inf)->buflen); \
        (inf)->cur = (inf)->buf + len;                                                     \
    }
...
pg_parser_Datum tsqueryout(pg_parser_Datum attr)
{
    ...
    nrm.buflen = 32;
    if (!pg_parser_mcxt_malloc(PGFUNC_TSQUERY_MCXT, (void**)&(nrm.buf), sizeof(char) * nrm.buflen))
    {
        return (pg_parser_Datum)0;
    }
    nrm.cur = nrm.buf;
...
static void infix(INFIX* in, int32_t parentPriority, bool rightPhraseOp)
{
    if (in->curpol->type == QI_VAL)
    {
        ...
        RESIZEBUF(in, curpol->length * (character_encoding_max_length(CHARACTER_UTF8) + 1) + 2 + 6);
    ...
    else
    {
        ...
        nrm.op = in->op;
        nrm.buflen = 16;
        pg_parser_mcxt_malloc(PGFUNC_TSQUERY_MCXT, (void**)&(nrm.buf), sizeof(char) * nrm.buflen);
        nrm.cur = nrm.buf;
